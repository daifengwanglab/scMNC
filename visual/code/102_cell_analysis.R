...
load('data/patch-seq-ephys-rpkms.rda')
medata=read.csv('data/patch-morph-ephys-features.csv',header=T,check.names = F)
gf_type = read.csv("data/DER-21_Single_cell_markergenes_UMI.csv",stringsAsFactors = F,header=T)

#feature selection ef
edata=edata[,!colnames(edata)%in%c('ADP (mV)','Latency (ms)')];nf = 12

cellnames=intersect(edata$`name sample`,colnames(gdata))
ef=edata[,-c(1,ncol(edata),ncol(edata)-1)];rownames(ef)=edata$`name sample`;ef=ef[cellnames,]
...
PCA_res_e = prcomp(ef,rank=3,retx=T)$x
