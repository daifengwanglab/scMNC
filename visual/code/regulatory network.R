...
# Read data & filter
expMat = read.csv("data/expMat.csv")
NMA_result = read.csv("data/ef_NMA.csv")
...
expMat = read.csv("data3/20200513_Mouse_PatchSeq_Release_count.v2.csv")
NMA_result = read.csv("data3/ef_NMA.csv")
expMat = expMat[!duplicated(expMat[,1]),]
rownames(expMat) = expMat[,1];expMat =as.matrix(expMat[,-1])
expMat = expMat[,NMA_result$cellnames[NMA_result$gmm_cluster == 1]]
