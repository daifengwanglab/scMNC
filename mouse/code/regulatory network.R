...
expMat = read.csv("../data/geneExp_motor.csv")
NMA_result = read.csv("../data/efeatures_NMA.csv")
expMat = expMat[!duplicated(expMat[,1]),]
rownames(expMat) = expMat[,1];expMat =as.matrix(expMat[,-1])
expMat = expMat[,NMA_result$cellnames[NMA_result$ttype == "Vip"]]

# Gene/cell filter/selection
ne_genes = read.csv("../data/neuronal_genes.csv")
TF_genes = read.csv("../data/TFgenes_simp.csv")
