...
```{r,warning=FALSE}
ef = read.csv("../data/motor_efeatures.csv",header = T,stringsAsFactors = F)
gf_exon = read.csv("../data/motor_gene_exon_counts.csv",header = T,stringsAsFactors = F)
gf_intron = read.csv("../data/motor_gene_intron_counts.csv",header = T,stringsAsFactors = F)
colnames(gf_exon)[2:ncol(gf_exon)] = sapply(colnames(gf_exon)[2:ncol(gf_exon)],function(x){na.omit(unlist(strsplit(x, "X")))[2]})
colnames(gf_intron)[2:ncol(gf_intron)] = sapply(colnames(gf_intron)[2:ncol(gf_intron)],function(x){na.omit(unlist(strsplit(x, "X")))[2]})
...
#remove NA 
ef = na.omit(ef)
gf_intron = gf_intron[,colSums(is.na(gf_intron)) == 0]
gf_exon = gf_exon[,colSums(is.na(gf_exon)) == 0]

#merge gf
gf_intron = gf_intron[,intersect(colnames(gf_intron),colnames(gf_exon))]
gf_exon = gf_exon[,intersect(colnames(gf_intron),colnames(gf_exon))]

#neuronal genes
gf = rbind(gf_exon,gf_intron)
gf = gf[!duplicated(gf[,1]),]
#write.csv(gf,"../data/geneExp_motor.csv",row.names = F)
...
genenames = na.omit(rownames(gf)[match(genenames,toupper(rownames(gf)))])
cellnames = intersect(cellnames,colnames(gf))
cellnames = intersect(cellnames,rownames(ef))
t_type = meta$`RNA family`[match(cellnames,meta$Cell)]
t_type_spec = meta$`RNA type`[match(cellnames,meta$Cell)]

# X,Y
ef = ef[cellnames,]
gf = data.matrix(gf[genenames,cellnames])

meta_filt = meta[match(cellnames,meta$Cell),]
#write.csv(meta_filt,"../data/meta_NMA.csv",row.names = FALSE)
#write.csv(ef,"../data/e-features.csv",row.names = FALSE)
#write.csv(gf,"../data/geneExp-fliered.csv",row.names = T)
```
...
dist =c()
for (i in 1:5){
  res = Dim_red(ef,gf,method = method[i],class = t_type,cellnames =cellnames,d=3L,k_NN=2L,k_medoid=5L)
...
#e-features NMA
pdf("rplot.pdf")
colpal = c(colorRampPalette(c("brown", "yellow"))(5),
           colorRampPalette(c("orange", "green"))(2),
           blues9[c(3,7)],
           colorRampPalette(c("red", "orange"))(8),
           colorRampPalette(c("pink", "purple"))(8),
           colorRampPalette(c("blue", "gray"))(3))
...
```{r}
main = c("Lamp5","Pvalb","CT","ET","Sst","Vip","IT")
for (i in main){
  png(filename = paste0("figure/sub-t-types-motor/",i," sub-t-types.png"))
  points3D(x=-NMA_res_e[t_type == i,]$Val0, y=-NMA_res_e[t_type == i,]$Val1, z=-NMA_res_e[t_type == i,]$Val2,
           pch = 19,cex=1,bty="g",ticktype = "detailed", theta = 40, phi = 10,
           xlab = "",ylab = "",zlab = "",main = paste0(i," motor"),
...
```{r}
### MA ###
NMA_res_sst = Dim_red(ef_sst,gf_sst,method = method[4],cellnames =cellnames_sst,d=3L,k_NN=2L,k_medoid=5L)
...
ggplot(data = ef_sst_new,mapping = aes(x=Val0,y=Val1,col=Rebound..mV.))+geom_point()
ef_new = cbind(ef,NMA_res_e)
ggplot(data = ef_new,mapping = aes(x=Val0,y=Val1,col=Max.number.of.APs))+geom_point()
```
...
#diffe.markers
write.csv(diffe.markers,"../data/diff_ef.csv",row.names = FALSE)
